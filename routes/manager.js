...
router.get('/view-salaries', function viewSalaries(req, res) {
    var userChunks = [];
    var salaryChunks = [];
    var totaldaysChunks = [];
    var totalworkingChunks = [];
    User.find({$or: [{type: 'employee'}, {type: 'project_manager'}]}).sort({_id: -1}).exec(function getUser(err, docs) {
        if (err) {
             console.log(err);
        }
    ...
    setTimeout(getUserSalaries, 900);
        ...
        setTimeout(getTotaldays, 900);

        function getTotaldays(){

            function callback(i) {
                if (i < userChunks.length) {
                    Attendance.find({employeeID: userChunks[i]._id, month: new Date().getMonth() + 1}, function (err, totaldays) {
                        console.log(i);

                        if (err) {
                            console.log(err);
                        }
                        totaldaysChunks.push(totaldays.length);
        ...
        setTimeout(getTotalworking, 900);

        function getTotalworking(){

            function callback(i) {
                if (i < userChunks.length) {
                    Attendance.find({employeeID: userChunks[i]._id}, function (err, totalworking) {
                        console.log(i);
                        if (err) {
                            console.log(err);
                        }
                       totalworkingChunks.push(totalworking.length);
                        callback(i + 1);
                    });
                }
            }
        ...
        function render_view() {
            ...
            res.render('Manager/view-salaries', {
                title: 'List Of Salaries', csrfToken: req.csrfToken(),
                users: userChunks, salary: salaryChunks, totalday: totaldaysChunks, totalworkings:totalworkingChunks, userName: req.session.user.name
            });
...
router.post('/set-employee', function setEmployee(req, res) {

    User.findById(req.body.managerid, function (err, us) {
        if (err) {
            console.log(err)
        }
        console.log(us);
        us.type = 'employee';
        us.save(function setEmployeeErr(err) {
            if (err) {
                console.log(err);
            }
            res.redirect('/manager/view-employees')
